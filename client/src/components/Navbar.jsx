  ...
  return (
    <header className="sticky top-0 z-30 bg-ink-950/95 backdrop-blur border-b border-ink-700">
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
        <a href="/" className="font-mono text-white text-sm tracking-tight" aria-label="FlowState — home">
          FLOWSTATE
        </a>
      ...
      {open && (
        <div id="mobile-nav" className="md:hidden border-t border-ink-700 px-6 py-4 flex flex-col gap-4">
          {NAV_SECTIONS.map(({ id, label }) => (
            <a key={id} href={`#${id}`} className={linkClass(id)} onClick={close}>
              {label}
            </a>
          ))}
          <a href="#contact" className={linkClass("contact")} onClick={close}>Contact</a>
        </div>
      )}
    </header>
  );
